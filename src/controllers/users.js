...
const userEdit = async (req, res) => {
  ...
  if (telefone.length < 10 || telefone.length > 11) {
    return res
      .status(404)
      .json("O campo telefone precisa ter 10 ou 11 dígitos (Incluindo o ddd)");
  }
  ...
  try {
    ...
    const updateUserProfile = await knex("usuarios")
      .update({ nome, email, senha: encryptedPassword, cpf, telefone })
      .where("id", usuario.id);
...
const getUser = async (req, res) => {
  ...
  const userData = await knex("usuarios").where("usuarios.id", usuario.id);
